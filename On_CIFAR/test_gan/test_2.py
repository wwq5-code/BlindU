...
class Generator(nn.Module):
    def __init__(self, resnet, nc=3, nz=100, ngf=64):
        ...
        self.resnet = resnet
        self.linear = nn.Linear(1402, 6384)
        self.tanh = nn.Tanh()
    ...
    def forward(self, noise, grad):
        ...
        input = concatenated_tensor.view(-1, 1, 128, 128)
...
attacker = Generator(resnet=resnet18).to(device)
...
for epoch in range(epochs):
    for i, (data) in enumerate(loader):
        ...
        z = torch.randn(batch_size, 10000, device=device)
