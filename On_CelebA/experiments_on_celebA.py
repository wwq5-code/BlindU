...
class Mine2(nn.Module):
    ...
    def forward(self, noise, sample):
        ...
        x = F.leaky_relu(x_noise + x_sample + self.fc1_bias, negative_slope=2e-1)
        x = F.leaky_relu(self.fc2(x), negative_slope=2e-1)
        x = F.softplus(self.fc3(x)) #, negative_slope=2e-1
        return x
...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        approximator = resnet18(3, 2)  #LinearModel(n_feature=3 * 7 * 7)
        explainer = resnet18(3, 3 * 7 * 7 * 2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=3 * 7 * 7, n_output=3 * 32 * 32)
        lr = args.lr
    elif dataset == 'FORREC':
...
def unlearning_frkl_compressed(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            index = index + 1
            train_bs = train_bs+1
            if acc_back < 0.05 and train_type == 'NIPSU':
                break
...
class CelebAAttrWrapper(Dataset):
    """
    一个包装类，将 torchvision.datasets.CelebA 返回的 (img, attr_vector)
    转换为 (img, single_attr)，并且提供 dataset.targets 以兼容其它代码。
    """

    def __init__(self, root, split='train', target_type='attr', transform=None, download=False, attr_index=20):
        self.celeba = CelebA(root=root,
                             split=split,
        ...
        # 预先将所有 y 保存到一个列表中，并命名为 self.labels
        # 这样就可以把它暴露为 targets 属性
        self.labels = []
        for i in range(len(self.celeba)):
            _, attr_vector = self.celeba[i]
            self.labels.append(attr_vector[self.attr_index])

    def __getitem__(self, index):
        # 从 self.celeba 中取出图像及其 40 维属性向量
        img, attr_vector = self.celeba[index]
        # 只取第 attr_index 个属性作为标签
        y = attr_vector[self.attr_index]
        return img, y
    ...
    def targets(self):
        """
        让外部代码可以通过 dataset.targets 访问到所有标签，
        这些标签是一维的（只有第 attr_index 个属性）。
        """
        return self.labels
...
if args.dataset == 'MNIST':
    ...
    attr_index = 20  # sex attribute index
    #/kaggle/input/celeba/   '../data/CelebA'
    data_path = '/kaggle/input/celeba/' # '../data/CelebA' #
    train_set = CelebAAttrWrapper(root=data_path,
                                  split='train',
                                  target_type='attr',
                                  transform=train_transform,
                                  download=False,
                                  attr_index=attr_index)
    test_set = CelebAAttrWrapper(root=data_path,
                                 split='test',
                                 target_type='attr',
                                 transform=train_transform,
                                 download=False,
...
poison_data, poison_targets = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
                                                            base_label=1,
                                                            trigger_label=0, poison_samples=poison_samples,
...
if args.dataset == 'MNIST':
   ...
   # data_reshape = remaining_set.reshape(len(remaining_set), 3, 32, 32)
    temp_img = torch.empty(0, 3, 32, 32).float().cuda()
    temp_label = torch.empty(0).long().cuda()
    for step, (x, y) in enumerate(train_set_loader):
        x, y = x.to(args.device), y.to(args.device)
...
vibi_f_frkl_ss_sampled_w, optimizer_frkl_ss_sampled_w = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled_w,
                                                          dataloader_remain, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
vibi_f_frkl_ss_sampled, optimizer_frkl_ss_sampled = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled,
                                                          dataloader_remain, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
print_multal_info(vibi_f_frkl_ss_sampled_w, dataloader_erase, dataloader_dp_sampled_w, args)

print('vibi_f_frkl_ss_sampled mutual info')
print_multal_info(vibi_f_frkl_ss_sampled, dataloader_erase, dataloader_dp_sampled, args)
