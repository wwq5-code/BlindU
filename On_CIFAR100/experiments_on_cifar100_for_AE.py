    ...
    def get_image_label(self, ):
        if args.dataset == "MNIST":
            ...
            for id in self.idxs:
                ...
                for id in self.idxs:
                    image, label = self.dataset[id]
                    image, label = image.to(args.device).reshape(1, 3, 32, 32), torch.tensor([label]).long().to(
                        args.device)
                    # print(label)
                    # label = torch.tensor([label])
                    temp_img = torch.cat([temp_img, image], dim=0)
...
#--gpu 0 --num_users 10 --dataset CIFAR100 --local_bs 100 --num_epochs 20 --beta 0.01 --lr 0.0005 --max_norm 1 --erased_portion 0.3 --erased_local_r 0.06
# --epsilon 1.0 --dp_sampling_rate 0.601 --dimZ 49 --unlearning_learning_rate 2 -- mi_rate 4 --mi_epoch 40 --reverse_rate 0.1 --kld_r 1.0 --unl_conver_r 2 --hessian_rate 0.000000005
...
def args_parser():
    ...
    parser.add_argument('--dataset', choices=['MNIST', 'CIFAR10','CIFAR100'], default='CIFAR100')
    parser.add_argument('--local_bs', type=int, default=100, help='Size of local batch')
    parser.add_argument('--num_epochs', type=int, default=20, help='Set the training epochs')
    parser.add_argument('--cuda', action='store_true')
    parser.add_argument('--beta', type=float, default=0.01, help='beta in objective J = I(y,z) - beta * I(x,z).')
    parser.add_argument('--lr', type=float, default=0.0005, help='set the learning rate')
    ...
    parser.add_argument('--unlearn_learning_rate', type=float, default=2, help='Unlearning update ratio')
    ...
    parser.add_argument('--reverse_rate', type=float, default=0.1, help='the rate of the reversed KLD, previously used')
    ...
    parser.add_argument('--hessian_rate', type=float, default=0.000000005, help='set the hessian unlearning rate during the hessian unlearning updating')
...
def test_category_acc(model, data_loader, args):
    ...
    model.eval()
    # Initialize counters
    correct_by_category = [0 for _ in range(20)]
    total_by_category = [0 for _ in range(20)]
...
class Mine1(nn.Module):
    ...
    def __init__(self, noise_size=3*7*7, sample_size=16 * 5 * 5, output_size=1, hidden_size=128):
        super().__init__()
...
class Mine2(nn.Module):
    ...
    def __init__(self, noise_size=3*7*7, sample_size=3*7*7, output_size=1, hidden_size=128):
...
class VIBI(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_z1 = logits_z.view(logits_z.size(0), 3, 7, 7)

            logits_y = self.approximator(logits_z1)  # (B , 10)
            ...
            # logits_z = logits_z.reshape((B,8,8,8))
            logits_z1 = logits_z.view(logits_z.size(0), 3, 7, 7)
            logits_y = self.approximator(logits_z1)
            return logits_y
        elif mode == 'test':
...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        approximator = resnet18(3, 20) #n_output=20
        explainer = resnet18(3, 3 * 7 * 7 * 2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=3 * 7 * 7, n_output=3 * 32 * 32)
        lr = 3e-4
    elif dataset == 'for_reconstruction':
        approximator = resnet18(3, 10)  #LinearModel(n_feature=3 * 7 * 7)
        explainer = resnet18(3, 3 * 7 * 7 * 2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=3 * 7 * 7, n_output=3 * 32 * 32)
        lr = args.lr
...
def learning_train(dataloader_full, model, loss_fn, reconstruction_function, args, epoch, mu_list,
                   sigma_list, train_loader, learn_model_type ,train_bs):
    ...
    for step, (x, y) in enumerate(dataloader_full):
        ...
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=args.max_norm, norm_type=2.0, error_if_nonfinite=False)
        optimizer.step()
    ...
    return model, mu_list, sigma_list, train_bs
...
def unlearning_frkl_compressed(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            # x2 = torch.rand(len(x2), 3, 32, 32).to(args.device)
            logits_z_e2, logits_y_e2, x_hat_e2, mu_e2, logvar_e2  = vibi_f_frkl(x2, mode='forgetting')
            ...
            M = Mine2(noise_size=3*7*7, sample_size=3*7*7)
            M.to(args.device)
            ...
            #unlearning_item = args.beta * KLD_mean2.item() + mi -  args.unlearn_learning_rate * H_p_q.item()- args.reverse_rate * (log_z.item() + log_y.item())
            ...
            # why we don't choose the former one because the former one is hard to unlearn backdoor
            # original methods
            ...
            total = unlearning_item + learning_item + 100 # expected to equal to 0
            if unlearning_item <= - 100:# have approixmate to the retrained distribution and no need to unlearn
                unl_rate = 0.0
            else:
                unl_rate = (unlearning_item +100) / total
            ...
            if train_type == 'VIBU':
                ...
                loss =  args.mi_rate * mi - args.unlearn_learning_rate * H_p_q
            elif train_type == 'VIBU-SS':
                ...
                loss = ( args.mi_rate * mi -  args.unlearn_learning_rate * H_p_q) * unl_rate + self_s_rate * args.self_sharing_rate * ( args.beta * KLD_mean2 + H_p_q2)  # args.beta * KLD_mean - H_p_q + args.beta * KLD_mean2  + H_p_q2 #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
            ...
            optimizer_frkl.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(vibi_f_frkl.parameters(), max_norm=args.max_norm, norm_type=2.0, error_if_nonfinite=False)
            optimizer_frkl.step()
            acc_back = (logits_y_e.argmax(dim=1) == y).float().mean().item()
            acc = (logits_y_e2.argmax(dim=1) == y2).float().mean().item()
            ...
            # print(f"Batch {epoch + 1}: Gradients of each parameter:")
            # for name, param in vibi_f_frkl.named_parameters():
            ...
            metrics = {
                'unlearning_item': unlearning_item,
                'learning_item': learning_item,
                'l2_norm_unl': l2_norm_unl.item(),
                'l2_norm_s': l2_norm_ss.item(),
                'acc': acc,
                'loss': loss.item(),
                'BCE': BCE.item(),
                'H(p,q)': H_p_q.item(),
...
def print_multal_info(vibi_f_frkl_ss, dataloader_erase,dataloader_sample, args):
    ...
    for i in range(mutual_training_round):
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_sample):
            t_round = t_round - 1
            if t_round < 0 : break
            x, y = x.to(args.device), y.to(args.device)
...
def vib_compressed_data_reconstruct(vibi, reconstructor, dataloader_erase,erasing_set,  dataloader_dp_sampled, dataloader_dp_sampled_w, args):
    vib_for_rec = copy.deepcopy(vibi).to(args.device)
    reconstructor = resnet18(3, 3 * 32 * 32)
    reconstructor = reconstructor.to(args.device)
    optimizer_recon = torch.optim.Adam(reconstructor.parameters(), lr=args.lr)
    ...
    # dataloader_erase = DataLoader(erasing_set, batch_size=args.batch_size, shuffle=False)

    optimizer_recon = torch.optim.Adam(reconstructor.parameters(), lr=args.lr)

    reconstruction_function = nn.MSELoss(size_average=False)

    # prepare_compress(copy.deepcopy(vibi).to(args.device), args, dataloader_erase_single)
    final_round_mse = []
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        reconstructor.train()
        step_start = epoch * len(dataloader_erase)
        for step, (x, y) in enumerate(dataloader_erase):
            x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
            # x = x.view(x.size(0), -1)
            logits_z, logits_y, x_hat, mu, logvar = vib_for_rec(x, mode='forgetting')  # (B, C* h* w), (B, N, 10)

            logits_z = logits_z.view(logits_z.size(0), 3, 7, 7)
            x_hat = torch.sigmoid(reconstructor(logits_z.detach()))  # .detach()
            x_hat = x_hat.view(x_hat.size(0), -1)
            x = x.view(x.size(0), -1)
            # x = torch.sigmoid(torch.relu(x))
            BCE = reconstruction_function(x_hat, x)  # mse loss
            ...
            if epoch == args.num_epochs - 1:
                final_round_mse.append(BCE.item())
            if step % len(train_loader) % 2300 == 0:
                print("loss", loss2.item(), 'BCE', BCE.item())

    print("final_round training mse", np.mean(final_round_mse))
    ...
    for (x, y), (x2, y2) in zip(dataloader_dp_sampled, dataloader_erase):
        ...
        if step % len(train_loader) % 600 == 0:
            print('epoch', epoch, "loss", loss2.item(), 'BCE', BCE.item())

    print("final_round mse of without replacement", np.mean(final_round_mse) / args.local_bs)
    mse_wo_final = np.mean(final_round_mse) / args.local_bs
    final_round_mse = []
    ...
    mse_w_final = np.mean(final_round_mse) / args.local_bs
    show_cifar(x_hat)
    for step, (x, y) in enumerate(test_loader, start=step_start):
        ...
        x = x.view(x.size(0), -1)
        break
    return mse_w_final, mse_wo_final
...
def prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase,dataloader_remain,  logs, reconstructor,  reconstruction_function,optimizer_recon, explainer_type, init_epoch, loss_fn, args,valid_acc, learn_model_type):
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        if learn_model_type == 'nips':
            train_epoch = args.num_epochs-15
        else:
            train_epoch = args.num_epochs
        for epoch in range(init_epoch, init_epoch + train_epoch):
            vibi.train()
        ...
        print()
        print("total_training_running time ", total_running_time)
        print('mu', np.mean(mu_list), 'sigma', np.mean(sigma_list))

        test_acc_of_org = valid_acc
        back_acc_of_org = backdoor_acc

        final_round_mse = []
        for epoch in range(init_epoch, init_epoch + args.num_epochs):
            vibi.train()#reconstructor.train()
            step_start = epoch * len(dataloader_erase)
            for step, (x, y) in enumerate(dataloader_erase, start=step_start):
                x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
                #x = x.view(x.size(0), -1)
    ...
    return vibi, test_acc_of_org, back_acc_of_org, train_bs, total_running_time
...
def reconstruct_Attack(vibi, reconstructor, reconstruction_function, args, dataloader_erase_single, test_loader):
    ...
    print("final_round mse", np.mean(final_round_mse))
    mse_w_final_vib = np.mean(final_round_mse) / args.local_bs
    # for step, (x, y) in enumerate(test_loader, start=step_start):
    #     x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
    #     if y!=3:continue
    #     # x = x.view(x.size(0), -1)
    ...
    # show_cifar(x_hat)
    # show_cifar(x)
    return reconstructor, mse_w_final_vib
...
# args.epsilon = 1.0
args.dp_sampling_size = int(32*32*args.dp_sampling_rate)
#
...
# args.mi_rate = 4
# args.mi_epoch = 40
# # args.unlearning_rate_for_cu = 1
# args.unlearn_learning_rate = 2  # when args.erased_local_r=0.02, set
...
args.self_sharing_rate = args.unlearn_learning_rate*10 # when args.erased_local_r=0.02, set args.self_sharing_rate = args.unlearn_learning_rate*1
# args.unl_conver_r = 2
...
poison_data, poison_targets = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
                                                            ...
                                                            batch_size=args.local_bs, args=args, add_backdoor=1, dp_sample=0)


sampled_data, sample_labels = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
                                                            base_label=1,
                                                            trigger_label=2, poison_samples=poison_samples,
                                                            batch_size=args.local_bs, args=args,  add_backdoor=1,dp_sample=1)


sampled_data_w, sample_labels_w = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
                                                            base_label=1,
                                                            trigger_label=2, poison_samples=poison_samples,
                                                            batch_size=args.local_bs, args=args,  add_backdoor=1,dp_sample=2) #dp_sample=2 with replacement
...
dataloader_full = DataLoader(poison_trainset, batch_size=args.batch_size, shuffle=True)  # poison_trainset

dataloader_remain = DataLoader(remaining_set, batch_size=args.batch_size, shuffle=True)
dataloader_dp_sampled = DataLoader(dp_sample_set, batch_size=args.batch_size, shuffle=True)
dataloader_dp_sampled_w = DataLoader(dp_sample_set_w, batch_size=args.batch_size, shuffle=True)
dataloader_erase = DataLoader(erasing_set, batch_size=args.batch_size, shuffle=True)
...
vibi, test_acc_of_org, back_acc_of_org, train_bs_of_org, total_training_time_of_org = prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase, dataloader_remain, logs, reconstructor, reconstruction_function, optimizer_recon, explainer_type, init_epoch, loss_fn, args, valid_acc,  learn_model_type)
...
print('hessian_mu')
mi_of_hessian_unl = print_multal_info_for_hessian(copy.deepcopy(vibi_f_hessian).to(args.device), dataloader_erase,dataloader_erase, args)

print('gradient vibi')
mi_of_grad_hbfu = print_multal_info_for_hessian(copy.deepcopy(vibi).to(args.device), dataloader_erase,dataloader_erase, args)



# print('vibi with mutual info')
...
vibi_f_nipsu, optimizer_nipsu, valid_acc_nipsu,  backdoor_acc_nipsu, train_bs_nipsu = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_erase, dataloader_remain, loss_fn,
                                                    reconstructor,
...
print()
print("start VIBU-SS with replacement dp sampling drop")
#dataloader_dp_sampled

vibi_f_frkl_ss_sampled_w, optimizer_frkl_ss_sampled_w, valid_acc_mcfu_w,  backdoor_acc_mcfu_w, train_bs_mcfu_w = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled_w,
                                                          dataloader_remain, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
#test_category_acc(vibi_f_frkl_ss_sampled_w, test_loader, args)


print()
print("start VIBU-SS without replacement dp sampling drop")
#dataloader_dp_sampled

vibi_f_frkl_ss_sampled, optimizer_frkl_ss_sampled, valid_acc_mcfu_wo,  backdoor_acc_mcfu_wo, train_bs_mcfu_wo = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled,
                                                          dataloader_remain, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
print('vibi with mutual info')
mi_of_w = print_multal_info(copy.deepcopy(vibi).to(args.device), dataloader_erase,dataloader_dp_sampled_w, args)

print('vibi wo mutual info')
mi_of_wo = print_multal_info(copy.deepcopy(vibi).to(args.device), dataloader_erase,dataloader_dp_sampled, args)




mse_w_final_vib, mse_wo_final_vib = vib_compressed_data_reconstruct(vibi, reconstructor, dataloader_erase,erasing_set,  dataloader_dp_sampled, dataloader_dp_sampled_w, args)

print('mse_w_final_vib, mse_wo_final_vib', mse_w_final_vib, mse_wo_final_vib)
...
reconstructor_grad, final_round_mse_on_whole_test_set = reconstruct_Attack(copy.deepcopy(vibi_train_for_inf).to(args.device), reconstructor_grad, reconstruction_function, args, test_loader_single, test_loader_single)
print('on only erased set')
reconstructor_grad, final_round_mse_on_only_erased_set = reconstruct_Attack(copy.deepcopy(vibi_train_for_inf).to(args.device), reconstructor_grad, reconstruction_function, args, dataloader_erase_single, test_loader_single)




print('prepare normal VBU training')
learn_model_type = 'nips'
vibi_for_nips, test_acc_of_vbu_org, back_acc_of_vbu_org, train_bs_vbu_org, total_training_time_of_vbu_org = prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase, dataloader_remain, logs, reconstructor, reconstruction_function, optimizer_recon, explainer_type, init_epoch, loss_fn, args,valid_acc,  learn_model_type)


print('vibi_for_nips mutual info')
mi_of_vbu = print_multal_info(vibi_for_nips, dataloader_erase,dataloader_erase, args)
#
# print('vibi_f_nipsu mutual info')
# print_multal_info(vibi_f_nipsu, dataloader_erase,dataloader_erase, args)




print('vibi_f_frkl_ss_sampled_w mutual info')
mi_of_mcfu_w_after_unl = print_multal_info(vibi_f_frkl_ss_sampled_w, dataloader_erase, dataloader_dp_sampled_w, args)

print('vibi_f_frkl_ss_sampled mutual info')
mi_of_mcfu_w_after_unl = print_multal_info(vibi_f_frkl_ss_sampled, dataloader_erase, dataloader_dp_sampled, args)

test_category_acc(vibi_f_frkl_ss_sampled, test_loader, args)
...
print("Privacy leak attacks of Origin:", final_round_mse_on_whole_test_set) #final_round_mse_on_whole_test_set
print("Backdoor Acc. of Origin:", back_acc_of_org)
print("Acc. on test dataset of Origin:", test_acc_of_org)
...
print("Privacy leak attacks of HBFU:", final_round_mse_on_only_erased_set) #final_round_mse_on_only_erased_set
...
print("Privacy leak attacks of VBU:", 0.0)  # because the server can get the erased samples on to implement VBU, therefore the recover mse is 0
