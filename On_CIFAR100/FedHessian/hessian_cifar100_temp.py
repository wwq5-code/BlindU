...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        approximator = LinearModel(n_feature=8*8*8, n_output=20)
        explainer = resnet18(3,  8*8*8*2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=8*8*8, n_output=3 * 32 * 32)
        lr = 3e-4
...
def print_multal_info_for_hessian(vibi_f_frkl_ss, dataloader_erase, dataloader_sampled, args):
    ...
    t_round = int(len(dataloader_erase) / total_erased_rate * 0.1)*2
...
if args.dataset == 'MNIST':
    ...
    dataset_train = CIFAR100Coarse('../../data/cifar', train=True, transform=train_transform, download=True )
    dataset_test = CIFAR100Coarse('../../data/cifar', train=False, transform=test_transform, download=True )
...
elif args.model == 'z_linear' and args.dataset == 'CIFAR100':
    # net_glob = models.resnet18().to(args.device)
    net_glob, lr = init_vibi(args.dataset)
    retrian_net, lr = init_vibi(args.dataset)
else:
...
unlearn_nips, acc_hbfu, back_acc_hbfu, unl_round = unlearning_net_global(unlearn_nips, idxs_local_dict, args, dataset_test, erased_perm, poison_testset)
...
print("Privacy leak attacks of HBFU:", "attack MSE is achieved on experiments_on_cifar100_for_AE.py") #final_round_mse_on_only_erased_set
