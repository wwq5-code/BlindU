...
def unlearning_frkl_compressed(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain,
                               loss_fn,
                               reconstructor, reconstruction_function, test_loader, train_loader, train_type):
    logs = defaultdict(list)
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            KLD_mean_z_and_z_e = 0.5 * torch.mean(
                logvar_e2 - logvar_f + (torch.exp(logvar_f) + (mu_f - mu_e2).pow(2)) / torch.exp(logvar_e2) - 1).to(
                args.device)
            ...
            if train_type == 'VIBU':
                ...
                loss = (
                                   args.mi_rate * mi - args.unlearn_learning_rate * H_p_q) * unl_rate + self_s_rate * args.self_sharing_rate * (
                                   args.beta * KLD_mean2 + H_p_q2)  # args.beta * KLD_mean - H_p_q + args.beta * KLD_mean2  + H_p_q2 #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
            ...
            optimizer_frkl.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(vibi_f_frkl.parameters(), max_norm=args.max_norm, norm_type=2.0,
...
def reconstruct_Attack(vibi, reconstructor, reconstruction_function, args, dataloader_erase_single, test_loader):
    ...
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_erase_single, start=step_start):
            ...
            x_hat = x_hat.view(x_hat.size(0), -1)
            # print(x_hat.shape)
            x_in_2 = x_in_2.view(x_in_2.size(0), -1)
...
#                         print(name)
            ...
            logits_z, logits_y, x_hat, mu, logvar = reconstructor(logits_z1.detach(), mode='forgetting')
            # torch.sigmoid(reconstructor(logits_z))
            x_hat = x_hat.view(x_hat.size(0), -1)
            # print(x_hat.shape)
            # x = x_in_2.view(x_in_2.size(0), -1)
            # x = torch.sigmoid(torch.relu(x))
            BCE = reconstruction_function(x_hat, x_in_2)  # mse loss
            loss2 = BCE
...
args.self_sharing_rate = args.unlearn_learning_rate*20 # when args.erased_local_r=0.02, set args.self_sharing_rate = args.unlearn_learning_rate*1
args.unl_conver_r = 2
...
# reconstructor_grad, lr = init_vibi('for_reconstruction') # resnet18(3, 3 * 32 * 32)  #LinearModel(n_feature=96*args.dimZ*2, n_output=28 * 28)
...
# reconstruction_function = nn.MSELoss(size_average=False)
# reconstructor_grad = reconstruct_Attack(copy.deepcopy(vibi).to(args.device), reconstructor_grad, reconstruction_function, args, dataloader_erase_single, test_loader_single)
