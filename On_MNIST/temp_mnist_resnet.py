...
class Mine1(nn.Module):
    ...
    def __init__(self, noise_size=3 * 49, sample_size=28*28, output_size=1, hidden_size=128):
...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        explainer = resnet18(1, 3 * 7 * 7 * 2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=3 * 7 * 7, n_output= 28 * 28)
        lr = args.lr
...
def learning_train(dataset, model, loss_fn, reconstruction_function, args, epoch, mu_list,
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1, norm_type=2.0, error_if_nonfinite=False)
        optimizer.step()
...
def unlearning_frkl_compressed(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            x2 = torch.rand(len(x2), 1,  28, 28).to(args.device)
            ...
            M = Mine1(noise_size=3 * 49, sample_size=3 * 49)
            M.to(args.device)
            ...
            total = unlearning_item + learning_item + 10 # expected to equal to 0
            if unlearning_item <= - 10:# have approixmate to the retrained distribution and no need to unlearn
                unl_rate = 0.0
            else:
                unl_rate = (unlearning_item + 10) / total
            ...
            optimizer_frkl.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(vibi_f_frkl.parameters(), 1, norm_type=2.0, error_if_nonfinite=False)
            #torch.nn.utils.clip_grad_norm_(vibi_f_frkl.parameters(), 5, norm_type=2.0, error_if_nonfinite=False)
            optimizer_frkl.step()
            acc_back = (logits_y_e.argmax(dim=1) == y).float().mean().item()
            acc = (logits_y_e2.argmax(dim=1) == y2).float().mean().item()
...
def unlearning_frkl_train(vibi, dataloader_erase, dataloader_remain, loss_fn, reconstructor, reconstruction_function,
    ...
    if args.dataset=="MNIST":
        reconstructor_for_unlearning = LinearModel(n_feature=3*49, n_output=28 * 28)
...
def print_multal_info(vibi_f_frkl_ss, dataloader_erase,dataloader_remain, args):
    reconstructor = LinearModel(n_feature=3 * 49, n_output=28 * 28)
    ...
    valid_acc = test_accuracy(vibi_f_frkl_ss, test_loader, args, name='vibi valid top1')
...
def reconstruct_show(vibi, mu_list, sigma_list, M, M_opt):
    print('mu', np.mean(mu_list), 'sigma', np.mean(sigma_list))
    #
    final_round_mse = []
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
...
def prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase,dataloader_remain,  logs, reconstructor,  reconstruction_function,optimizer_recon, explainer_type, init_epoch, loss_fn, args,valid_acc, learn_model_type):
    ...
    # reconstruct_show(vibi, mu_list, sigma_list, M, M_opt)
    return vibi
...
args.unl_r_for_bayesian = args.unlearn_learning_rate*0.1
...
sampled_data, sample_labels = create_backdoor_train_dataset(dataname=args.dataset, train_data=train_set,
                                                            base_label=1,
                                                            trigger_label=2, poison_samples=poison_samples,
                                                            batch_size=args.local_bs, args=args,  add_backdoor=1,dp_sample=1) #dp_sample=1 without replacement
...
print_multal_info(vibi, dataloader_erase,dataloader_remain, args)
...
print('mutual info for vibi')
print_multal_info(vibi, dataloader_erase,dataloader_remain, args)

print('mutual info for vibi_for_nips')
print_multal_info(vibi_for_nips, dataloader_erase,dataloader_remain, args)
...
print_multal_info(vibi_f_nipsu, dataloader_erase,dataloader_remain, args)
...
print_multal_info(vibi_f_frkl_sampled_w, dataloader_erase,dataloader_remain, args)
...
print_multal_info(vibi_f_frkl_sampled, dataloader_erase,dataloader_remain, args)
