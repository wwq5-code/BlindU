...
def unlearning_frkl_compressed(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase, dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        # print("backdoor_acc", backdoor_acc_list)
        # print("acc_test: ", acc_test)
        if backdoor_acc < 0.1 and train_type == 'NIPSU':
            # print()
            # print("end unlearn, train_bs", train_bs)
            break
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        for iter in range(args.num_epochs):  # self.args.local_ep
            ...
            for (images, labels), (images2, labels2) in zip(dataloader_erase, dataloader_remain2):
                ...
                # for name, param in vibi_f_hessian.explainer.named_parameters():
                #     # if name == 'linear_head.weight':
...
def prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase,dataloader_remain,  logs, reconstructor,  reconstruction_function,optimizer_recon, explainer_type, init_epoch, loss_fn, args,valid_acc, learn_model_type):
    ...
    return vibi, mse_w_final, mse_wo_final, test_acc_of_org, back_acc_of_org, train_bs, total_training_time


def prepare_reconstruction_attack(vibi, reconstructor, reconstruction_function, args, dataloader_erase_single, test_loader):
    #
...
# we only calculate time here to calculate the training time of each epoch. We will calculate the unlearning time by calculating the time of each epoch * training epochs
# it is beacuse for the convenient we put the compression operation before unlearning at the same loop, whie in fact the compressed data should be independently calculated by user.
# so we record the training epochs as train_bs in the later unlearning training
# start_time = time.time()
learn_model_type = 'vib'

vibi, mse_w_final_vib, mse_wo_final_vib, test_acc_of_org, back_acc_of_org, train_bs_of_org, total_training_time_of_org = prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase, dataloader_remain, logs, reconstructor, reconstruction_function, optimizer_recon, explainer_type, init_epoch, loss_fn, args, valid_acc,  learn_model_type)
...
test_loader_single = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=1)
print("grad on test")
reconstructor_grad, final_round_mse_on_whole_test_set = prepare_reconstruction_attack(copy.deepcopy(vibi).to(args.device), reconstructor_grad, reconstruction_function, args, test_loader_single, test_loader_single)
print("grad on erased")
reconstructor_grad, final_round_mse_on_only_erased_set = prepare_reconstruction_attack(copy.deepcopy(vibi).to(args.device), reconstructor_grad, reconstruction_function, args, dataloader_erase_single, dataloader_erase_single)
...
vibi_f_frkl_sampled_w, optimizer_frkl_sampled_w, valid_acc_mcfu_w,  backdoor_acc_mcfu_w, train_bs_mcfu_w = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled_w,
                                                          dataloader_clean_erase, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
vibi_f_frkl_sampled, optimizer_frkl_sampled, valid_acc_mcfu_wo,  backdoor_acc_mcfu_wo, train_bs_mcfu_wo = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_dp_sampled,
                                                          dataloader_clean_erase, loss_fn,
                                                          reconstructor,
                                                          reconstruction_function, test_loader, train_loader, train_type='VIBU-SS')
...
vibi_for_nips, mse_w_final_vbu, mse_wo_final_vbu, test_acc_of_vbu_org, back_acc_of_vbu_org, train_bs_vbu_org, total_training_time_of_vbu_org = prepare_learning_model(dataloader_full, train_loader, test_loader, dataloader_erase, dataloader_remain, logs, reconstructor, reconstruction_function, optimizer_recon, explainer_type, init_epoch, loss_fn, args,valid_acc,  learn_model_type)
...
mi_of_vbu_after_unl = print_multal_info(vibi_f_nipsu, dataloader_erase, dataloader_erase, args)

print('mutual vibi_f_frkl_sampled_w for vibi_f_frkl_sampled_w')
mi_of_mcfu_w_after_unl = print_multal_info(vibi_f_frkl_sampled_w, dataloader_erase, dataloader_erase, args)

print('mutual vibi_f_frkl_sampled for vibi_f_frkl_sampled w/o')
mi_of_mcfu_w_after_unl = print_multal_info(vibi_f_frkl_sampled, dataloader_erase, dataloader_erase, args)
